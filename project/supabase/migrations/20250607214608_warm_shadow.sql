/*
  ...
  1. 既存のusersテーブルを削除してSupabase Authと連携
  2. プロファイル情報用のprofilesテーブルを新規作成
  3. 認証関連のポリシーとトリガーを追加
  4. 既存テーブルのuser_id参照をauth.uid()に更新
*/

-- 既存のusersテーブルを削除（外部キー制約があるため段階的に削除）
...
-- プロジェクトテーブル（user_idをauth.uid()参照に変更）
